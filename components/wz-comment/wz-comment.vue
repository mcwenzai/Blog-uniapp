			...
			<block v-else>
				<view class="comlist_title">
					...
					<view class="classification">
						<view @tap="showRecommendList" class="newest" :class="isNewest?'active':''">推荐</view>
						<view @tap="showNewestList" class="newest" :class="isNewest?'':'active'">最新</view>
					</view>
				</view>
					...
					<view class="comment-reply-item" v-for="(item,index) in commentList" :key="index">
						<view class="comment-header">
							<view class="comment-header-left">
								<image catchtap="gotouser" class="comment-avatar" :src="item.author.avatar">
								</image>
							...
							<view @tap="tapcommentlike(item.id,index)" class="comment-header-right">
								<view :class="detail.islike===false?'icon i-dianzan1':'icon i-dianzan2'"></view>
