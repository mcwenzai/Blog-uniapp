    ...
    <view class="scroll-wrap" :style="{zIndex: zIndex}">
        ...
        >
            <view
                ...
                @click="onClickScrollItem(index, item, $event)"
            >{{item.sortName}}</view>
...
export default {
    ...
    async mounted() {
        ...
        query.exec((res) => {
            res && res.length && res[0].forEach((element, i)=> {
                const { left } = element || {}
                const obj = {
                    left: left,
                }
    ...
    methods: {
        onClickListItem(index, data) {
            ...
            const currentListItem = this.scrollViewLeftArr.get(index)
            this.position = currentListItem.left - (this.windowWidth / 2 - 30)
            this.choseInd = index
            this.$emit('onValueChange', {currentInd: index, currentItem: data})
            this.$emit('onListShow', true)
        },
        onClickScrollItem(index, data, event) {
            this.choseInd = index
